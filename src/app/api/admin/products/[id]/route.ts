...
/* ========= تجميعة قراءة المنتج (GET) مع تحسين الوسوم ========= */
async function buildProductDetails(db: any, product_id: string) {
  ...
  /* ===== الوسوم (تحسين مهم): join مباشر لإرجاع {id,name} ===== */
  ...
  return {
    ...
    price: {
      ...
      currency: mainPrice?.currency ?? "SAR",
    },
    sale_price:
      mainPrice?.sale_price != null
        ? { amount: mainPrice.sale_price, currency: mainPrice?.currency ?? "SAR" }
        : null,
    sale_end: mainPrice?.ends_at ?? null,
    main_cost_price: mainCost,
    main_sku: mainSku, // ← مهم لواجهة البطاقة
    quantity,
...
export async function PATCH(
  ...
  try {
    ...
    // 3) tags  ✅ (يدعم tagIds أو tags بالأسماء) — منطقك كما هو
