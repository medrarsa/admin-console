...
// يدعم العربية + الأرقام + الشرطة ويمنع الرموز الغريبة (موجود عندك)
...
// [SEO-SLUG] نسخة للسلاج: تشذيب الحواف + حد طول
function slugifySeo(val: string) {
  return val
    .toString()
    .trim()
    .replace(/\s+/g, "-")
...
// يضمن تفريد الـSKU بإضافة -01, -02 ...
...
// [SEO-SLUG] تفريد السلاج على seo_pages (entity_type='product')
async function ensureUniqueProductSlug(base: string, client: any) {
...
const ok = (data: any, status = 200, headers?: Record<string, string>) =>
  new NextResponse(JSON.stringify({ success: true, data }), {
    status,
    headers: {
...
type CardRow = {
  ...
  variants_total_qty: number | null;
  base_price_fallback: number | null;
  base_qty_fallback: number | null;
};

function encodeCursor(row: CardRow) {
  return Buffer.from(
...
function decodeCursor(s?: string | null) {
  ...
  try {
    ...
    return o && o.created_at && o.id
      ? (o as { created_at: string; id: string })
      : null;
  } catch {
    return null;
  }
...
/* =========================================
   GET /api/admin/products  — (LIST as Cards + Cursor)
   يعتمد على الـ VIEW: public.product_card_view
   ========================================= */
export async function GET(req: NextRequest) {
  try {
    ...
    // نقرأ من الـ VIEW مباشرة
    let query = supabase
      .from("product_card_view")
      .select(
        "id,name,status,created_at,image_url,variants_price_min,variants_price_max,variants_total_qty,base_price_fallback,base_qty_fallback"
      )
      .order("created_at", { ascending: false })
      .order("id", { ascending: false })
      .limit(limit + 1); // واحدة زيادة لمعرفة هل فيه صفحة تالية

    if (cursor) {
      // كُرسر مركّب: created_at ثم id
      query = query
    ...
    if (error) {
      // في حال الـ VIEW غير موجود:
      if (
        error.message?.toLowerCase?.().includes("relation") &&
        error.message.includes("product_card_view")
      ) {
        return fail(
          "المنظر product_card_view غير موجود. أنشئ الـ VIEW أولاً (شوف SQL اللي أعطيتك) ثم أعد المحاولة.",
          500,
          { hint: "create view public.product_card_view ..." }
        );
      }
    ...
    const items = page.map((r) => ({
      ...
      variants_price_min: r.variants_price_min,
      variants_price_max: r.variants_price_max,
      variants_total_qty: r.variants_total_qty ?? 0,
      base_price_fallback: r.base_price_fallback,
      base_qty_fallback: r.base_qty_fallback ?? 0,
    }));

    return ok({ items, nextCursor }, 200);
  } catch (e: any) {
    return fail(e?.message || "تعذّر جلب قائمة المنتجات", 400);
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    /* 1.1) [SEO-SLUG] توليد سلاج + تفريده + upsert في seo_pages */
    try {
      const base = slugifySeo(prodIns.name);
      const seoSlug = await ensureUniqueProductSlug(base, admin);
      const { error: seoErr } = await admin
        .from("seo_pages")
        .upsert(
          { entity_type: "product", entity_id: productId, slug: seoSlug, is_active: true },
          { onConflict: "entity_type,slug" }
        );
      if (seoErr) warnings.push(`SEO slug warning: ${seoErr.message}`);
    } catch (e: any) {
      warnings.push(`SEO slug generation failed: ${String(e?.message || e)}`);
    }
    ...
    for (const s of skusInput) {
      ...
      if (optionValueIds.length) {
        const { error } = await admin.from("variant_option_values").insert(
