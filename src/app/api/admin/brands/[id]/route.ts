...
export async function DELETE(
  ...
  await supabase.from("seo_pages").delete().match({ entity_type: "brand", entity_id: id });

  // حاول حذف الماركة
  let { error: delErr } = await supabase.from("brands").delete().eq("id", id);

  // معالجة قيود FK (products.brand_id)
  if (delErr && (delErr as any).code === "23503") {
    if (!force) {
      return NextResponse.json(
        { message: "لا يمكن الحذف: الماركة مرتبطة بمنتجات. أعد المحاولة مع force=true أو عطّلها بدلًا من الحذف.", code: delErr.code },
        { status: 409 }
      );
    }
    // force=true => فك الربط ثم احذف
    await supabase.from("products").update({ brand_id: null }).eq("brand_id", id);
    ({ error: delErr } = await supabase.from("brands").delete().eq("id", id));
  }
