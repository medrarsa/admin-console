// src/app/(store)/products/[slug]/product-client.tsx
"use client";
...
export default function ProductClient({ slug, initial }: Props) {
  ...
  const [activeImg, setActiveImg] = React.useState<string | null>(
    initial?.image ?? null
  );
  ...
  if (loading || !p) {
    return (
      <div className="mx-auto max-w-screen-lg px-3 py-8 text-center text-sm text-neutral-500">
        جارٍ التحميل…
      </div>
    );
  }
  ...
  keys.forEach((k) => {
    const vals = Array.from(
      new Set(attrs.map((o) => o[k]).filter(Boolean) as string[])
    );
    ...
    <div className="mx-auto max-w-screen-lg px-3 py-4 md:py-8">
      <div className="grid gap-4 md:grid-cols-2 md:gap-6">
              ...
              {p.gallery.map((g, i) => (
                <button
                  ...
                  className={`relative aspect-square overflow-hidden rounded-xl ring-1 ring-black/5 ${
                    activeImg === g ? "outline outline-2 outline-black/40" : ""
                  }`}
                >
        ...
        <div className="flex flex-col">
          <div className="mb-2">
            {p.brand_name && (
              <div className="text-xs text-gray-500">{p.brand_name}</div>
            )}
            <h1 className="text-lg font-semibold leading-6 md:text-2xl">
              {p.name}
            </h1>
          </div>
          ...
          <div className="mt-1 flex items-baseline gap-3">
            {hasSale ? (
              <>
                <div className="text-2xl font-semibold text-gray-900 md:text-3xl">
                  {priceText}
                </div>
          ...
          <div className="mt-6 divide-y rounded-2xl border">
            ...
            <details className="group">
              ...
              <div className="px-4 pb-4 text-sm text-gray-700 leading-6">
                شحن مجاني للطلبات فوق 199 ريال. استرجاع خلال 7 أيام بشرط سلامة
                التغليف.
              </div>
            </details>
          </div>
        ...
        <div className="mx-auto flex max-w-screen-sm items-center gap-2 px-3 py-3">
          ...
          >
            ...
            <svg viewBox="0 0 24 24" className="h-5 w-5 text-gray-700">
              <path
                fill="currentColor"
                d="M12.1 21.35l-1.1-1.01C5.4 15.36 2 12.28 2 8.5 2 6 4 4 6.5 4c1.74 0 3.41.81 4.5 2.09C12.09 4.81 13.76 4 15.5 4 18 4 20 6 20 8.5c0 3.78-3.4 6.86-8.99 11.84l-1.11 1.01z"
              />
            </svg>
          </button>
          <button
