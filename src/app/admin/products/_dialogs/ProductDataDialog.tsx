...
export default function ProductDataDialog({
  ...
  /* ✅ لا نُسند السعر الأساسي كخصم افتراضي */
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // حقن ماركة القاعدة داخل الخيارات فورًا
        if (d?.brand?.id && d?.brand?.name) {
          setBrandOpts((prev) =>
            prev.some((o) => o.id === d.brand.id)
              ? prev
        ...
        // إظهار اسم الماركة في الاستعلام أول مرة
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // إبقاء الماركة المختارة ظاهرة حتى لو البحث ما رجّعها
        const opts = (options as BrandOption[]) ?? [];

        const merged: BrandOption[] =
          brandId && brandName
            ? opts.some((o: BrandOption) => o.id === brandId)
              ? opts
              : [{ id: brandId, name: brandName }, ...opts]
            : opts;
  ...
  function buildPatch() {
    ...
    if (salePriceStr.trim() !== "" && saleNum !== undefined) {
      ...
      // لو المستخدم مسح الحقل، امسح الخصم
      patch.salePrice = null;
    }

    patch.discountEnd = discountEnd || null;

    // ربط الماركة: brandId أولاً، وإلا اسم حر
  ...
  async function saveNow() {
    try {
      ...
      if (fresh?.brand?.id && fresh?.brand?.name) {
        setBrandOpts((prev) =>
          prev.some((o) => o.id === fresh.brand.id)
            ? prev
            : [{ id: fresh.brand.id, name: fresh.brand.name }, ...prev]
        );
      }
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-zinc-200/70 bg-white/80 p-4">
              <div className="mb-1 flex items-center justify-between">
                <label className="block text-xs text-zinc-600">الماركة</label>
                {brandId ? (
              ...
              <div className="relative">
                ...
                >
                  ...
                  {/* Fallback: لو عندي brandId والاسم، والخيار غير موجود بعد */}
                  {brandId &&
                    brandName &&
                    !brandOpts.some((o) => o.id === brandId) && (
                ...
                <div className="mt-1 text-[11px] text-zinc-500">
                  اختر من القائمة أو أدخل اسمًا حرًّا وسيتم ربطه بالمنتج
                  تلقائيًا.
                </div>
              </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200/70 bg-white/80">
            ...
            <div className="p-4">
              <TinymceEditor
                ...
                apiKey="qsytkn3snr04rlev0qo8rzجرب"
                value={descHTML}
